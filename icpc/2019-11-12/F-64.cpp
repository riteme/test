...
#define CNT(x) __builtin_popcountl(x)
#define LOGW 6
#define W (1 << LOGW)
#define MOD (W - 1)
#define NMAX 65536
...
struct Bits {
    ...
    int count() const {
        if (L + 1 == R) {
            if (L >= MAXR) return 0;
            int len = r - l;
            u64 mask = len < W ? (1LL << len) - 1 : -1LL;
            u64 bits = (w[L] >> (l & MOD)) & mask;
            return CNT(bits);
        }

        int xl = L, xr = min(R, MAXR);
        //int ret = 0;
        //for (int i = L; i < R && i < MAXR; i++)
        //    ret += CNT(w[i]);
        return xl < xr ? suf[xl] - suf[xr] : 0;
    }
...
static int dfn[NMAX + 10], low[NMAX + 10], tm;
