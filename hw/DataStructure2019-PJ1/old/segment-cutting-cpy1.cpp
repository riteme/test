...
void erase(Segment *s, int &n, int i) {
    if (i < n)
        memmove(s + i, s + i + 1, sizeof(Segment) * (n - i));
    n--;
}
...
void sort_segments() {
    ...
    for (int i = 1, j = 1, p = 0; i <= m || j <= K; ) {
        ...
        if (i > m)
            ...
            u = s[i++];
        else if (p && j <= K && ts[p].size() >= LARGE_THRESHOLD)
            u = ns[j++];
        else if (s[i] < ns[j])
            u = s[i++];
        else
