...
struct Solver {
    ...
    i64 query(int s, int t, EnableList en, const vector<int> &disabled) {
        if (s == t)
            return 0;
        return search(disabled.empty() ? G : G0, s, t, en);
    }
...
int main() {
    ...
    auto span = duration_cast<microseconds>(t2 - t1).count();
    DEBUG("init: %.3lfms", static_cast<double>(span) / 1000);
