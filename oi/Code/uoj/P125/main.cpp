...
int main() {
    ...
    for (int i = 1; i <= m; i++) {
        ...
        for (int j = 1; j <= n; j++) {
            for (int k = n; k >= 1; k--) {
                chkmax(f[last][0][j][k], f[last][0][j][k + 1]);
            }
        }
        ...
        for (int k = 1; k <= n; k++) {
            ...
            for (int j = 1; j <= k + 1; j++) {
                ...
                chkmax(maxv, f[last][1][j][k]);
                chkmax(f[last][1][j][0], maxv);

                if (j <= k) {
                    chkmax(r, f[last][0][j][k + 1]);        
                    chkmax(r, f[last][1][j][0]);
        ...
        for (int k = 1; k <= n; k++) {
            for (int j = k; j >= 1; j--) {
                chkmax(f[last][1][j][k], f[last][1][j + 1][k]);
            }
        }
