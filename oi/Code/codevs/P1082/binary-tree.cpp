...
typedef pair<ntype, ntype> ResultType;

static ResultType query(Node *x, int left) {
    if (!x)
        return ResultType(0, 0);
    if (left <= x->left)
        return ResultType(x->sum.xw, x->sum.w);
    if (x->right < left)
        return ResultType(0, 0);

    ResultType lresult = query(x->leftchild, left);
    ResultType rresult = query(x->rightchild, left);
    ResultType result = ResultType(lresult.first + rresult.first,
                                   lresult.second + rresult.second);

    if (left <= x->x) {
        result.first += x->x * x->w;
        result.second += x->w;
    }
...
inline ntype evaluate(Node *x, int left) {
    ResultType a = query(x, left);
    return a.first - (left - 1) * a.second;
}
