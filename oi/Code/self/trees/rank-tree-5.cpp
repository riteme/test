...
struct Node {
    ...
    std::string to_string() {
        stringstream buffer;
        buffer << this << ": {key = " << this->key << ", left = " << this->left
               << ", right = " << this->right << "}";
...
static Node *_merge(Node *a, Node *b) {
    ...
    int sit1 = size_of(a->right) + b->size;
    int sit2 = size_of(b->left) + a->size;
    if (sit1 < sit2) {
        a->pushdown();
        // ap++;
...
Node *fold(Node *h) {
    int mid = h->size / 2;
    NodePair a = split(h, mid);
    return _merge(a.first, a.second);
}
