...
class HashTable {
 public:
    ...
    int operator[](const u64 key) const {
        int pos = key % MOD;

        for (auto &e : bucket[pos]) {
            if (e.first == key)
                return e.second;
        }  // foreach in bucket[pos]
...
static HashTable block;
static HashTable ground;
...
inline bool has(int x, int y) {
    return x < 1 || x > n || y < 1 || y > m || block[code(x, y)];
}
...
bool search(int u) {
    ...
    auto test = [](const int tx, const int ty) {
        ...
        int v;
        if ((v = block[p]) && !marked[v] && search(v))
            return true;
